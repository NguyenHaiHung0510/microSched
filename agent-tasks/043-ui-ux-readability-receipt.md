...
## CI integration correction, 2026-09-06

PR [204](https://github.com/NguyenHaiHung0510/microSched/pull/204), first head `c0a0b73`: [Migration QA run 34046486650](https://github.com/NguyenHaiHung0510/microSched/actions/runs/34046486650) failed **6 PG tests**, while 191 PG cases passed. The failing cases create/enable fixed reminders and received HTTP 500. Raw job log is retained as `output/task-043/ci-migration-first.log`. No failing gate was waived or merged.

T1 and the backend worker independently traced the result conversion to `dict(rows)`: a real SQLAlchemy `ChunkedIteratorResult` exposes `keys()` but does not implement mapping subscripting. The original list-backed fake did not model that protocol. New parameterized regressions invoke the actual projection helper with real SQLAlchemy results for empty history and a five-day dispatch anchor. Both produced the intended `TypeError` before correction: `sqlalchemy-red.log`, **2 FAIL**, exit 1. The fix iterates `(tracker_id, dispatched_on)` rows explicitly with a comprehension; SQL, recurrence, query count and privacy gates are unchanged. The existing full PG API suite remains the integration gate after this correction.

After correction, the complete database-free backend suite reports **396 PASS, 197 PG deselected**, exit 0 (`backend-sqlalchemy-green.log`). Ruff check passed; Ruff format initially identified mixed line endings from the edit, then both files were normalized and format check passed (`sqlalchemy-format-final.log`). Frontend source and captured appearance are unchanged by this correction.
