...
| **011c** | `subscription` + luồng gia hạn + **F6** (burn cố định/tháng) + `app_setting` tối thiểu (toggle hiện giá, ngưỡng "sắp hết hạn") + **seam định tuyến** | **Owner-approved; local acceptance PASS trên `b12bae6`.** GitHub/final-review/production gates chưa đóng. **KHÔNG có migration.** |
| **011b** | nhắc thuốc + nhắc sub hết hạn — Web Push, dispatch và confirmation | **Owner-approved; local acceptance PASS trên `b12bae6`.** Scheduler target là `011d`; GitHub/production/Web Push activation chưa verify. |
| **011d** | inprocess-cron-timer | **Owner-approved; local acceptance PASS trên `b12bae6`.** Phương án A mặc định disabled; migration/production/timer activation chưa verify. |
