...
| **019** | `TasksScreen.tsx` — sửa padding lệch "+ Thêm chi tiết" + tooltip subtask kiểu D tĩnh (eyebrow, đánh số tối đa 3 mục, "… và N mục nữa") | ⚠️ OPEN. Bug viền đã đóng độc lập trước khi spec chốt; task này chỉ còn 2 mục visual polish, không thêm tương tác vào tooltip. Chạy độc lập với `009`, không cần Docker/DB. Chi tiết: `agent-tasks/019-tasks-visual-polish.md`. |
