...
|---|---|
| **LAST VERIFIED** | 2026-08-09: Wave A đã closeout với R1/R2 **PASS WITH FIXES**, không có blocker; owner đã duyệt Wave B đúng **2 findings**. Security patch local `9259f98`; model override đã hoạt động. |
| **LIVE** | Production receipt cũ vẫn là `221d222…`; Wave A/Wave B và security patch local **chưa deploy**. Final review route owner chốt: **Gemini 3.6 Flash / high**. |
| **IN FLIGHT** | Integration closeout sau Wave A: chỉ một integrator xử lý đúng 2 findings đã duyệt; không thêm nested-delegation rule. Pilot chưa có final acceptance. |
| **BLOCKED** | **Không có blocker từ R1/R2.** Các cổng còn lại là acceptance kỹ thuật chưa được suy ra: fresh receipts sau Wave B, GitHub CI, merge/deploy, migration Neon, timer/VAPID activation và iPhone/Web Push. |
| **NEXT** | Chạy final review theo route owner đã chốt `Gemini 3.6 Flash/high`, sau đó fresh QA/CI/production gates; không quay lại model-override probe như một blocker. |
| **OPEN QA DEBT** | `010a`: iPhone file picker / FileReader. `011a`: Lane 4 iPhone vật lý. `011b`: Web Push trên thiết bị thật, VAPID/timer activation. `016`: còn 2 non-text contrast dưới WCAG 1.4.11. Pilot Wave A chưa có final acceptance. |
...
| **011a** | tracker tầng nền — `tracker_group`/`tracker`/`entry` CRUD + lưới ghi một chạm + dashboard A1–A4/F1–F5 | 📝 **Owner-approved theo handoff 2026-08-09**: qua phản biện T3 + T2, T1 vá từng finding. **KHÔNG có migration** — 4 bảng đã đủ cột từ `0001`; thấy "thiếu cột" là dấu hiệu đi chệch `tracking-brief.md`, dừng lại hỏi. **Bắt buộc có 008a** — `entry.amount` CHECK vô điều kiện, không ghi nổi plaintext. |
| **011c** | `subscription` + luồng gia hạn + **F6** (burn cố định/tháng) + `app_setting` tối thiểu (toggle hiện giá, ngưỡng "sắp hết hạn") + **seam định tuyến** | Spec: `agent-tasks/011c-subscription-renewal-settings.md` (viết 2026-08-01). **Owner-approved theo handoff 2026-08-09.** **KHÔNG có migration** — `subscription`/`app_setting` + 3 index đã đủ từ `0001`. **Chạy GIỮA `011a` và `011b`**. |
| **011b** | nhắc thuốc + nhắc sub hết hạn — hạ tầng Web Push (VAPID + service worker) + cron **3 khe** | Spec: `agent-tasks/011b-medication-reminder-webpush.md`, qua hai vòng adversarial review + fixes. **Owner-approved theo handoff 2026-08-09; pilot/final acceptance chưa đóng.** **Chạy CUỐI họ 011** (sau `011c`). |
| **011d** | inprocess-cron-timer | **Owner-approved theo handoff 2026-08-09** — Phương án A, mặc định disabled; pilot/final acceptance và activation gates chưa đóng |
