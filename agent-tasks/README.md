...
|---|---|
| **LAST VERIFIED** | T1 receipt **2026-08-15**: `develop` production ở `4b7db483025d8efacb78e44c2f01fdecfaad3dc4`; `/api/readyz.commit` khớp exact SHA, DB `up`. |
| **LIVE** | Private Unlock contrast guardrail + poll cadence đã merge/deploy qua **#131**; production Chrome **partial PASS 2026-08-15** cho unlock/lock và tương phản PIN input ở 390×844 + 1280×800. iPhone/Safari thật và tương phản throttled badge trên production chưa verify. QA execution spec 017 đã merge/deploy qua **#133**; đây là spec receipt, chưa phải product acceptance. Task 020 đã merge/deploy qua **#132**; migration `0009` đã áp tay và raw catalog xác nhận trước merge. 011 integration/activation/observability đã merge qua **#115 / #117 / #125**. Security #113 đã release `v0.4` qua #116. Dependency #119–#124 và Alembic 1.19 (#122, sau reconciliation #127) đã merge/deploy. |
...
| **IN FLIGHT** | PR **#134** sửa focus-return dialog Tracker mới. PR vẫn cần exact-head independent review + CI trước merge; local receipt không phải production/iPhone acceptance. |
...
| **012** | cutover migration + verify | **DRAFT — PR #143**, exact-head `ccd3196` đã nhận T3 P2 BLOCK; follow-up docs khóa deterministic `TIME` encoding và fail-closed non-NULL note VECTOR pre-state, còn chờ exact-head adversarial re-review + owner strategic sign-off. Không giao implementation/rehearsal/cutover. Contract giữ source encrypted restore-tested dump; no full Neon trash dump; two-phase signed manifest; purge domain mock atomically nhưng preserve operational state; exact canonical proof cho mapped và purge-only end state; 022 production viewability hard prerequisite. `020` đã LIVE nên không còn blocker riêng. Source archived rows vẫn fail-closed pending owner decision. |
| **016** | private unlock — PIN 6 số + Argon2id, `private_until` TTL 36 phút | ✅ **DONE (code/live 2026-07-31); contrast guardrail fixed trong PR #131, independent browser QA local PASS 2026-08-15.** PR #67 (code, live `de95d30`) + PR #68 (decision note). Hai viền non-text từng dưới WCAG 1.4.11 nay được Playwright đo thật: PIN focus `4,15:1`, PIN thường `4,99:1`, badge throttled `4,43:1`; 6/6 browser cases pass ở 390×844 + 1280×800. Production Chrome partial PASS cho unlock/lock và PIN input ở 390×844 + 1280×800; iPhone/Safari thật và throttled badge trên production chưa verify, nên chưa gọi QA debt đóng hoàn toàn. |
| **020** | ba cột giữ dữ liệu app cũ: `task.completed_at` · `note.pinned` · `note.priority` | ✅ **DONE + LIVE 2026-08-15 — PR #132, merge `ff11dd3`.** Migration `0009` đã được áp tay lên Neon trước merge; raw catalog xác nhận đủ ba cột và `ck_note_priority_values`. Hậu merge: CI/deploy xanh, production `/api/readyz.commit=ff11dd3cf9224d16842225aeda0aa6cb880044e5`, `db=up`, Fly Machine `sin` health passing. `012` không còn bị chặn bởi `020`, nhưng vẫn giữ mọi gate riêng của cutover. |
| **021** | cân bằng TanStack Query polling theo mức cần đồng bộ | ✅ **DONE + LIVE 2026-08-16 — PR #138, merge `d345c0e`.** Task 1s; Notes/Tracker/Subscription 15s; Calendar/session/reminder-confirm không interval; hidden/background dừng; query mới mặc định không poll; focus/foreground và mutation vẫn refetch. Spec: `agent-tasks/021-balanced-query-polling.md`; supersede policy global 1s lịch sử ở 018 §2.6. |
| **022** | Task day timeline + full-history viewability | ✅ **OWNER-APPROVED DECISION 2026-08-16; implementation PENDING.** Timeline mặc định hôm nay−3…hôm nay+3, overdue/undated groups, pinned chỉ trong group, completed gấp được, block 7 ngày và full-history path không page numbers. Hard prerequisite UX/API trước cutover 012: bỏ fixed 100 bằng date-range + cursor bounded, vẫn giữ Task poll 1s chỉ khi màn visible. Spec: `agent-tasks/022-task-day-timeline.md`; **không** tự duyệt/thi công cutover 012. |
