...
| **010a** | calendar tầng nền — migration `0005` + parser `.ics` + CRUD nguồn/buổi + màn danh sách | 📝 **Tách đôi 2026-08-01** (chủ chốt): gộp một lô thì to ~2,5× `009` và nửa sau đụng cả domain task. Spec: `agent-tasks/010a-calendar-import-crud.md`, qua 2 lượt `adversarial_review` (T3+T2), **chủ đã duyệt 2026-08-01 — sẵn sàng giao Codex**. **`.xlsx` bị bỏ hẳn** — lịch thi của trường xuất được `.ics` với đủ metadata, còn file `.xlsx` thật thì cột ngày lẫn lộn chuỗi với serial Excel. Giải luôn `migration-mapping-brief.md` §3 (121 dòng lịch lệch) bằng cách import lại file gốc. |
| **010b** | calendar phần nhìn — lịch **cuộn liên tục** kiểu Outlook + mini-nav + đánh dấu ngày (`day_annotation`, riêng tư từ đầu) + dời việc | Spec: `agent-tasks/010b-calendar-scroll-view.md`, qua 2 lượt `adversarial_review` (T3+T2) + 3 mục quyết định thị giác chốt qua bản mẫu chạy được (ô ngày mobile = phương án C 2 chip, task hiện thành chip, `day_annotation` gác riêng tư). **Chủ đã duyệt 2026-08-01 — sẵn sàng giao Codex.** **Phụ thuộc cứng: chờ 010a merge + chạy thật trên production rồi mới bắt đầu thi công** (không phải chờ để viết spec — spec đã xong). |
| **011a** | tracker tầng nền — `tracker_group`/`tracker`/`entry` CRUD + lưới ghi một chạm + dashboard A1–A4/F1–F5 | 📝 **Tách ba 2026-08-01**: gộp cả họ vào một PR thì to hơn `009` + `010a` cộng lại, và phần nặng nhất (tiền mã hoá + cổng đọc qua cha) chìm giữa đống CRUD. Spec: `agent-tasks/011a-tracker-capture-dashboard.md`, qua phản biện T3 + T2, T1 vá từng finding. **DRAFT — chưa được chủ duyệt, đừng giao thi công.** **KHÔNG có migration** — 4 bảng đã đủ cột từ `0001`; thấy "thiếu cột" là dấu hiệu đi chệch `tracking-brief.md`, dừng lại hỏi. **Bắt buộc có 008a** — `entry.amount` CHECK vô điều kiện, không ghi nổi plaintext. |
| **011c** | `subscription` + luồng gia hạn + **F6** (burn cố định/tháng) + `app_setting` tối thiểu (toggle hiện giá, ngưỡng "sắp hết hạn") + **seam định tuyến** | Spec: `agent-tasks/011c-subscription-renewal-settings.md` (viết 2026-08-01). **DRAFT — chưa được chủ duyệt.** **KHÔNG có migration** — `subscription`/`app_setting` + 3 index đã đủ từ `0001`. **Chạy GIỮA `011a` và `011b`**, không phải cuối: `011b` nhắc cả thuốc lẫn sub, phần nhắc sub cần `subscription` tồn tại. **Vá 2 lỗ hổng giữa các spec** phát hiện lúc viết: (a) không lô nào nhận phần dựng **định tuyến** dù `011b` cần 2 deep link (app hiện chuyển màn bằng `useState`, không có `react-router`) — `011c` nhận, seam tự viết ~40 dòng; (b) `app_setting` **dùng chung** với `private_gate.py` (giữ hash PIN + throttle + TTL) nên CRUD tổng quát sẽ rò hash Argon2id của PIN 6 chữ số ⇒ bắt buộc allowlist hằng số. |
| **011b** | nhắc thuốc + nhắc sub hết hạn — hạ tầng Web Push (VAPID + service worker) + cron **3 khe** | Spec: `agent-tasks/011b-medication-reminder-webpush.md`, qua phản biện T3 + T2. **DRAFT — chưa được chủ duyệt.** **Chạy CUỐI họ 011** (sau `011c`). Nâng `devops-brief.md` §10 từ "một job mãi mãi" lên 3 job cố định — nhu cầu thật đổi (toa tạm thời ⇒ nhiều giờ nhắc/ngày), nhưng **vẫn không scan-liên-tục**: cron 5 phút/lần hỏi DB = **sự cố Neon 22/07 mặc áo khác** (nhịp tối thiểu cron đúng bằng cửa sổ idle Neon). |
