...
| 008d | 3 mục tồn từ security review toàn dự án (2026-07-23) | 📋 **có spec**. Mục 2 (gitleaks bỏ sót `NEON_OWNER_URL`) + mục 3 (`compare_digest` non-ASCII → 500) là việc máy móc, chạy lúc nào cũng được. **Mục 1 thì không:** `task` bắt mã hoá cả title khi private, `note` chỉ bắt body — hai câu trả lời khác nhau cho cùng một câu hỏi, chưa ai quyết. **Quyết trước 008** vì 008 đặt khuôn và bảng còn rỗng (sửa sau = migration có backfill). |
