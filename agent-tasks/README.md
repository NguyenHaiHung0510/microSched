...
| **019** | `TasksScreen.tsx` — sửa bug viền `border-transparent` bị `* {border-color}` không-layer đè + tooltip subtask đổi sang kiểu D (đánh số + "+N mục nữa" bấm được) | ⚠️ OPEN. Sinh từ phiên tư vấn taste UI (chủ dựng mockup so sánh, chốt trực tiếp trong chat) — 2/3 phát hiện hoá ra là bug CSS thật (Cascade Layers), không phải taste. Chạy độc lập với `009`, không cần Docker/DB. Chi tiết: `agent-tasks/019-tasks-visual-polish.md`. |
