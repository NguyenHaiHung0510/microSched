...
| **018** | Batch polish `TasksScreen.tsx` (tooltip bị cắt · vùng bấm cả thẻ · banner trễ hạn · 2 microcopy · `data-testid`) + `refetchInterval` đồng bộ đa thiết bị + **Playwright suite đầu tiên** | ✅ **Spec viết xong 2026-07-29** → `018-qa-polish-playwright.md`, đã qua **lượt phản biện T3** (`gemini-3.1-pro-high`, *"spec sai ở đâu"*): 5 finding fold, 1 bác — xem §7 của spec. **Còn chờ lượt T2** (*"spec không làm được ở đâu"*) rồi giao Codex. **Chạy TRƯỚC 009** để 009 kế thừa harness. ⚠️ Phần *khung QA* đã tách ra **`docs/qa-framework.md`** — nó là tài liệu dùng lại cho mọi slice, không phải việc thi công một lần. |
