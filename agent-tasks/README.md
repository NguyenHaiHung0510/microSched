...
|---|---|
| **LAST VERIFIED** | T1 receipt 2026-08-09: Wave A R1/R2 **PASS WITH FIXES**, không blocker; owner duyệt đúng 2 findings và Wave B fix commit `b12bae6` đã local. Security patch `9259f98` cũng local; model override hoạt động. |
...
| **IN FLIGHT** | Luna full acceptance cho integration đang chờ/đang chạy trên state sau `b12bae6`. Chưa gọi pilot final acceptance. |
| **BLOCKED** | **Không có blocker từ R1/R2.** Fresh integration acceptance, GitHub CI, merge/deploy, migration Neon, timer/VAPID activation và iPhone/Web Push vẫn là gate chưa đóng. |
| **NEXT** | Chờ fresh Luna full acceptance → push/open đúng 3 PR → chạy final reviews bằng route owner chốt **Gemini 3.6 Flash / high**. Chỉ sau các receipt đó mới đi tiếp merge/production gates. |
| **PILOT RECEIPT** | Nested Terra→Luna **FAIL** vì child không có `spawn_agent`; T1 chuyển sang flat orchestration. Luna security verifier pass các check áp dụng; T1 đã restore E2E-generated screenshot. Gitleaks working-tree scan vẫn đỏ do ignored `.venv` như handoff đã biết; history scan sạch. Đây chưa phải pilot final trước integration acceptance. |
...
| **011c** | `subscription` + luồng gia hạn + **F6** (burn cố định/tháng) + `app_setting` tối thiểu (toggle hiện giá, ngưỡng "sắp hết hạn") + **seam định tuyến** | Spec: `agent-tasks/011c-subscription-renewal-settings.md` (viết 2026-08-01). **Owner-approved theo handoff 2026-08-09; final acceptance chưa đóng.** **KHÔNG có migration** — `subscription`/`app_setting` + 3 index đã đủ từ `0001`. **Chạy GIỮA `011a` và `011b`**. |
| **011b** | nhắc thuốc + nhắc sub hết hạn — Web Push, dispatch và confirmation | Spec: `agent-tasks/011b-medication-reminder-webpush.md`, qua hai vòng adversarial review + fixes. **Owner-approved theo handoff 2026-08-09; final acceptance/activation chưa đóng.** Scheduler target là `011d` in-process timer. |
