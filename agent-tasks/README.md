...
| **018** | Batch polish `TasksScreen.tsx` (tooltip bị cắt · vùng bấm cả thẻ · banner trễ hạn · 2 microcopy · `data-testid`) + `refetchInterval` đồng bộ đa thiết bị + **Playwright suite đầu tiên** | ✅ **Spec sẵn sàng giao thi công (2026-07-29)** → `018-qa-polish-playwright.md`. Đã qua **cả hai lượt phản biện** theo khung hạng-đôi mới (`devops-brief.md` §7.3.i): **T3** (`gemini-3.1-pro-high`) 5 fold + 1 bác, **T2** (Codex) 2 fold + 1 làm rõ — xem §7 của spec. **Chạy TRƯỚC 009** để 009 kế thừa harness. ⚠️ Phần *khung QA* đã tách ra **`docs/qa-framework.md`** — tài liệu dùng lại cho mọi slice, không phải việc thi công một lần. |
| 009 | note slice | ⇢ song song được (điều kiện: `devops-brief.md` §8). **Việc kế tiếp: chi tiết hoá §2** — làm sau khi 018 xong. |
...
| **016** | private unlock — Argon2id, `private_until` TTL 15 phút | Thiết kế đã khoá ở `auth-brief.md` §3 (`argon2-cffi`, throttle 5 lần sai → khoá 15 phút, nút khoá-lại-ngay, badge trạng thái). **Chưa có spec thi công.** Đề xuất chạy sau 009 (áp dụng cho cả task lẫn note, dựng cổng một lần rẻ hơn dựng riêng rồi nới). **Bắt buộc xong trước phiên AI Bước 1** — ruleset R1–R7 buộc AI đi theo cổng private, không có cổng thì chọn provider mù. ⚠️ Toggle "riêng tư" **đã sống trên UI** ([`TasksScreen.tsx`](../frontend/src/TasksScreen.tsx)) mà chưa có endpoint mở khoá — task tạo riêng tư hôm nay biến mất khỏi mọi GET và **không có đường lấy lại** cho tới khi 016 xong. Cân nhắc khoá tạm toggle đó ở `TaskForm` trong lúc chờ. |
