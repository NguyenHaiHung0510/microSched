...
> **Trạng thái: FROZEN v1.0 / CHƯA CHẠY — 2026-09-20.** Owner đã duyệt tiến hành Bước 3.
...
- Spec ID: `mimi-p1-live-dogfood`; version: `1.0`; freeze date: `2026-09-20`.
...
| `MLD-DET-002` | A1 clarify | DET-001 | Missing-title Task intent | Run provider double returning clarification | Text terminal; zero tool | One assistant clarification, zero change set/task | UI shows question, Mimi `ready`; no preview | `B0-OFFLINE` | `test_mimi_openrouter.py` mapping + DB counts | Exact fixture rows | CHƯA CHẠY |
...
| `MLD-DET-007` | A1 canonicalization | DET-005 | Provider args with schedule sibling/status/id attempts | Normalize server-owned fields | Canonical `task.create.v1`; server owns ID, open status, STANDARD flag | Frozen args contain canonical values only | Preview equals canonical payload | `B0-OFFLINE` | Before/provider vs canonical diff | None | CHƯA CHẠY |
...
| `MLD-DET-009` | A1 concurrency | One pending preview | Stale conversation generation/frontier | Submit new/revision message with stale expected values | HTTP conflict; no provider dispatch | Existing pending preview preserved; zero new call/write | UI refreshes truth, no false success | `B0-OFFLINE` | Status/body, call count=0, DB before/after | Restore current generation | CHƯA CHẠY |
...
| `MLD-SSE-002` | A1 delta | SSE-001 | Split multibyte Vietnamese deltas | Stream and reassemble | Monotonic sequence; no duplicate/gap; UTF-8 intact | Buffered deltas encrypted at rest and bound to run/sequence/kind | Text appears once; no mojibake; avatar `thinking` | `B0-OFFLINE` | Wire excerpt, ciphertext query, UI text hash | Exact run rows | CHƯA CHẠY |
...
| `MLD-SSE-008` | A1 terminal truth | SSE-001–007 PASS | Failure/unknown/cancel/deadline fixtures | Terminalize each state | Wire event/state matches actual outcome; stream close alone never success | Canonical provider outcome only succeeded/failed/unknown; run state distinct | Correct controls/copy/avatar; no green success for unknown/failure | `B0-OFFLINE` | State matrix across wire/DB/UI | Restore fault controls | CHƯA CHẠY |
