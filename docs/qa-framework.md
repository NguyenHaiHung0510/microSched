...
5. **Post-cutover High-Fidelity & Migration Rehearsal trên Staging/Develop Neon Branch (📝 2026-08-26 cập nhật):**
   Sau cut-over, khi cần kiểm thử migration trên dữ liệu thật hoặc QA giao diện với số lượng bản ghi thật (51 notes, 223 tasks):
   - **🔒 Điểm dừng bắt buộc (Stop & Request Owner):** Agent **không có quyền** và **không được tự chạy lệnh** tạo/xóa/restore nhánh Neon (`neonctl`). Agent bắt buộc phải dừng lại và yêu cầu Owner:
     > *"Vui lòng lên Neon Console đồng bộ (Restore/Sync) nhánh `develop` từ `main` (Production) và xác nhận sau khi hoàn tất để tiếp tục."*
   - Sau khi Owner xác nhận, bắt buộc chạy `python -m scripts.prepare_qa_branch` để Data Scrubbing toàn diện:
     * Scramble text/markdown bảo tồn độ dài 1:1, khoảng trắng, định dạng danh sách và CJK/tiếng Việt.
     * Re-encrypt các trường private bằng `QA_ENCRYPTION_MASTER_KEY` (hoặc synthesize text nếu không có key) và đặt test PIN `123456`.
