...
**Intake không phải dispatch — chốt task contract trước khi viết.** Một cụm issue/ý tưởng thô chỉ là input để T1 triage read-only: phân loại domain, blast radius, dependency, trust boundary và xem nên gộp hay tách slice. Trước implementation hoặc repair đầu tiên, T1 trình chủ chốt một **task contract** vừa đủ cho việc đó: outcome mong muốn; scope/out-of-scope; dependency/hard start; topology + model/effort; quality/reliability axes; review posture; mức + nhịp báo cáo; loop budget và stop/re-scope trigger. `targeted` chỉ soi seam + regression đã nêu; `adversarial` thử mọi failure mode trong declared contract; `challenge` còn chủ động falsify assumption/scope/architecture và có thể trả việc về decision gate. Không có số vòng mặc định cho mọi task. Chủ có thể explicit delegate một khoảng linh hoạt cho T1 tự điều chỉnh; thay đổi vượt khoảng đó phải quay lại decision gate.

**Quality contract có trước handoff đầu tiên.** T1 tổng hợp trước các tiêu chí liên quan từ decision brief và risk inventory, thay vì để reviewer phát hiện dần rồi mới bổ sung prompt. Tùy task, contract cân nhắc happy path; error/retry; concurrency/idempotency; privacy/security; legacy/migration; UX/taste; evidence/runtime/device. Trục không liên quan phải được loại rõ, không sao chép boilerplate. Mức chi tiết tỷ lệ với blast radius, uncertainty và năng lực executor/reviewer: model yếu hoặc lane deterministic nhận oracle cụ thể hơn; model mạnh ở judgment lane được giữ quyền chọn phương pháp trong guardrail.

**Immutable handoff + consolidated finding ledger.** Review chỉ bắt đầu trên immutable commit/ref hoặc artifact digest đã freeze; writer không đổi target trong lúc reviewer đọc. Lượt `adversarial`/`challenge` phải đi hết mọi axis đã khai báo và trả **một ledger hợp nhất**, không dừng ở P1 đầu tiên, trừ P0 hoặc nguy cơ authority/irreversible buộc dừng ngay. Mỗi finding ghi `introduced | pre-existing | evidence-only | out-of-scope`, severity, evidence và suggested gate. Re-review đóng ledger cũ rồi đọc delta + regression surface; P1 mới phải nói rõ vì sao nó chưa thể/đã không được phát hiện ở lượt trước. Reviewer read-only, không biến review thành chuỗi drip-feed một finding mỗi vòng.

**Adaptive loop budget + efficiency gate.** Task contract đặt budget tương đối theo uncertainty, blast radius, model reliability, chi phí evidence và giá trị deadline — không hard-code “ba vòng”. Mỗi vòng bổ sung phải nêu expected information gain, phạm vi thay đổi và điều kiện đóng. Khi cùng một failure class tái diễn, target/spec tiếp tục drift, hoặc marginal verified progress không còn xứng time/token, T1 dừng auto-loop và trình chủ chọn `ship | defer | split | rewrite spec | change executor`. Số lần handoff tự nó không phải lỗi; thước đo là verified coverage/closure trên time + token. Việc tìm được bug thật không tự chứng minh workflow hiệu quả nếu một đường rẻ hơn đáng lẽ bắt được cùng bug sớm hơn.

**Tách gate, không đánh đồng debt với regression.** Product/code correctness, evidence/report completeness và runtime/device acceptance là các gate riêng, báo riêng `PASS | FAIL | BLOCKED | NOT_RUN`. Evidence-only/P2 hoặc flaky ngoài scope không tự mở lại product implementation, trừ khi nó làm lộ bug thật hoặc task contract đã chốt đó là blocker. Taste/evidence vẫn có thể là deliverable bắt buộc của task UI/QA; T1 phải chốt điều này trước execution, không nâng cấp muộn giữa loop.

**Policy core ngắn, task detail linh hoạt.** §7 chỉ giữ decision framework bền; model-specific prompt, exact command/oracle, cadence, artifact matrix và report depth nằm ở task contract/spec. Runtime Catalog và role profile có thể đổi mà không cần viết lại lifecycle. Sau terminal, T1 báo chủ một packet hợp nhất: outcome, verified receipts, debt/NOT_RUN, loop-cost so với budget, và khuyến nghị next decision — không biện hộ hiệu suất chỉ bằng số bug đã bắt.
...
**Milestone/event-driven, không forensic poll loop.** T1 chỉ báo chủ khi có dispatch/review receipt, blocker/decision, terminal CI/acceptance receipt hoặc timebox correction. Sự kiện terminal hoặc blocker phải báo ngay, không chờ cadence. Ngoài policy heartbeat giới hạn ở đoạn sau, T1 không chạy scheduled status/forensic/CI/deploy poll loop. Khi short timebox đã khai báo hết mà chưa có terminal outcome, T1 chỉ được **reassign hoặc reschedule trong nguyên objective/scope/authority**; ghi trigger + outcome receipt (vào `coordination_record` nếu lane có record) nêu boundary giữ nguyên và next gate. Silence/timebox không bao giờ là approval để âm thầm expand, cancel hay delete.

**Heartbeat quan sát live lane.** Khi còn bất kỳ subagent/external lane nào live và T1 nếu không có heartbeat sẽ idle, T1 tạo hoặc cập nhật **một** heartbeat cho mỗi target group: cadence cố định 5 phút; riêng browser/visual QA dài (kể cả Chrome QA) dùng 7–9 phút, mặc định 8 phút. Mỗi wake chỉ query tối đa một lần cho mỗi target; heartbeat chỉ quan sát/ghi observed receipt, không tự cấp authority, đổi gate hoặc mở rộng scope. Cadence hoặc stop condition do Owner chỉ định rõ luôn thắng default. Không tạo heartbeat trùng: phải update heartbeat đang có; hủy ngay heartbeat cũ khi mọi target terminal, bị supersede hoặc không còn cần quan sát. Một recurring monitor ngoài policy heartbeat này vẫn chỉ tạo theo yêu cầu rõ của Owner và phải có target, trigger từ yêu cầu Owner, frequency, stop condition; mỗi wake ghi trigger + observed receipt vào task/PR evidence. Mọi cadence 3/6/10/15/20 phút trong status/session receipt cũ chỉ là lịch sử, không phải policy hiện hành.

**Activation canary cho monitor.** T1 chỉ được báo monitor `ACTIVE` sau khi API view/update thành công, `next run` nằm đúng cadence và wake thật đầu tiên tới trong `cadence + 2 phút`; config ghi `ACTIVE` tự nó không phải receipt. Quá mốc hoặc API trả lỗi như `Transport closed` ⇒ báo ngay `MONITOR_FAILED`, chuyển sang bounded manual check khi T1 còn active và chỉ recreate sau khi automation service hồi phục.
