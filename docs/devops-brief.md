...
**Delegation không mở rộng authority.** Mỗi lane giữ nguyên scope/quyền của task cha; không có hai writer cùng sửa một worktree. Deploy, migration, secret-bearing flow và thao tác irreversible vẫn do T1 hoặc T2 mạnh thực hiện dưới đúng merge/approval gate.

**Kỷ luật chờ và quan sát.** Agent/CI đang chạy không tiến nhanh hơn vì T1 gọi `wait`, `gh pr checks` hay status poll song song; các lượt trùng chỉ tạo noise và đốt context. Commentary giữa các lần kiểm chỉ được gửi khi có receipt, blocker hoặc decision mới — không lặp lại trạng thái “vẫn đang chạy”. Quy tắc này không cấm executor tự làm việc giữa các event.

**Cadence mặc định của chủ.** Tính từ lúc giao agent, T1 kiểm đúng một lần gần các mốc **phút 1, 3, 5, 10, 15 và 20**; sau phút 20, kiểm khoảng **10 phút/lần** đến terminal state. Completion/blocker notification vẫn được xử lý ngay. Với PR checks, deploy và các external run tương tự, khoảng cách mặc định giữa hai lần kiểm là **khoảng 2 phút** đến terminal state. Một mốc chỉ có một status query; không gọi các check song song để hỏi cùng một trạng thái.
