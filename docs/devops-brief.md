...
**Kỷ luật chờ và quan sát.** Agent/CI đang chạy không tiến nhanh hơn vì T1 gọi `wait`, `gh pr checks` hay status poll liên tục; nhiều lượt poll song song còn chỉ tạo noise và đốt context. Lần check theo lịch đầu tiên là **sau khoảng 10 phút**. Chỉ check sớm hơn khi có completion/blocker notification tự phát, chủ yêu cầu, deadline gần đã biết, hoặc tín hiệu critical cần quyết định. Commentary giữa hai lần chỉ được gửi khi có receipt, blocker hoặc decision mới — không lặp lại trạng thái “vẫn đang chạy”. Quy tắc này không cấm executor tự làm việc giữa các event.

**Nghĩa vận hành của cadence do chủ yêu cầu.** “Check khoảng 10 phút một lần” hoặc “check sau khi xong” là lệnh tạo một **scheduled follow-up/heartbeat** bền gắn với task đang chạy trước khi T1 kết thúc lượt hiện tại — không phải lời hứa sẽ quay lại ở một chat đã đóng. Completion/blocker notification là điều kiện wake ngay; nếu không có event thì scheduled run mới thực hiện lần kiểm theo cadence. Khi đạt terminal receipt, blocker cần chủ quyết, hoặc task bị thay thế, T1 phải pause/xóa lịch. Nếu runtime không có cơ chế schedule phù hợp, nói rõ không thể bảo đảm lần check tương lai; không được viết “sẽ kiểm sau” rồi kết thúc.
