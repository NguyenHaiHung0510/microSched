...
- 📝 **2026-07-20 — ĐÃ CHỐT stack UI (phiên frontend):** React 19 + TypeScript + Vite 8 + Tailwind v4/shadcn-ui + TanStack Query v5; offline = **Dexie + outbox tự viết** (không sync-engine — bài toán multi-user conflict không tồn tại, tránh thêm mảnh rời). Chi tiết + lý do + phần còn mở có chủ đích (router/chart/web-push implementation — lúc scaffold/build) + hệ quả thiết bị chủ là iPhone: **`frontend-brief.md`**.
...
## 7. Auth — ✅ CHỐT 2026-07-20 → `auth-brief.md` (nội dung leaning cũ giữ bên dưới làm bối cảnh)
...
- 📝 **2026-07-20 — ✅ ĐÃ CHỐT (phiên auth):** leaning giữ nguyên thành quyết định — **Authlib** (login-only OIDC, allowlist env, Google app để Testing mode đủ); session = **bảng `session` server-side** trong Postgres (cookie token mờ lưu hash, TTL rolling 60–90 ngày); **private unlock = passphrase riêng Argon2id** mở theo session TTL 15′; kèm bộ luật **AI × private R1–R7** (AI đi theo cổng private của session; giữ luật không-embed/FTS trên cột mã hóa; private vào context ⇒ ép `zdr`/no-train cả cascade — mở rộng cấu hình privacy §8). Chi tiết + lý do + phương án đã loại: **`auth-brief.md`**.
...
- ~~Frontend stack cụ thể + thư viện sync offline-first~~ → ✅ chốt 2026-07-20, `frontend-brief.md`.
- ~~Auth: thư viện cụ thể, cookie/session store~~ → ✅ chốt 2026-07-20, `auth-brief.md`.
