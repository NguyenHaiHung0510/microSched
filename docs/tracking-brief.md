...
📝 **2026-07-24 — ⤴ ĐẢO: `task_item` gác bằng TRIGGER, KHÔNG phải app-layer.** Ở phiên tư vấn 008, chủ chọn **thêm trigger** (BEFORE INSERT/UPDATE trên `task_item` chặn `content` trần dưới task cha private; + BEFORE UPDATE OF `is_private` trên `task` chặn lật sang private khi còn item con trần). Lý do chủ nêu: *"đơn giản và chuẩn zero-trust"* — bất biến không vỡ âm thầm vì bug, và 009–012 **thừa hưởng bảo đảm DB miễn phí** thay vì mỗi slice tự chứng minh lại bằng test (mould-amortization). Bảng còn rỗng ⇒ chỉ là DDL thuần. **Không** chọn nhân-cờ-xuống-con (vẫn phạm K11 + đẻ hai-nguồn-sự-thật lệch nhau). ⚠️ `adversarial_review` khác-họ (24/07) bắt: trigger **KHÔNG tự đủ dưới concurrency** — READ COMMITTED cho hai transaction không thấy việc chưa-commit của nhau ⇒ store **phải** `SELECT … FOR UPDATE` dòng task cha ở đường toggle/ghi-item; trigger là lưới cuối, khoá-dòng là lưới chính. Chi tiết + thứ tự toggle 2 chiều: `agent-tasks/008-task-slice.md` §2.4–§2.5. *(Note app-layer phía trên GIỮ NGUYÊN làm hồ sơ — đây là đảo có lý do, không xoá kết luận cũ.)*
...
## 15. One-shot reminders — Owner approved 2026-09-07, Task 047

A task, calendar event or tracker can own one active one-shot reminder independently of tracker recurrence. An absolute time works even without a deadline. A relative reminder follows a task deadline or event start, before or after in minutes/hours/days. Date-only tasks and all-day events require an explicit Vietnam anchor clock. Tracker one-shots use absolute time. Recurrence interval zero is rejected; it is not a one-shot encoding.

Pending relative reminders follow source edits; a past/missing anchor or an uncertain in-flight send becomes `needs_reschedule`. Absolute schedules remain fixed. Sent/cancelled schedules never rearm when the source changes or reopens; a user-created new occurrence is explicit. Completion, deletion or existing event hiding cancels active reminders. The timer rehydrates durable state and receives source changes only after commit.

One-shot catch-up is 45 minutes inclusive, with at most four provider attempts and 30/120/600-second backoff. Later occurrences become missed. Push TTL cannot outlive that window. Generic notification text contains no source prose; parent visibility gates listing/detail/write/history. Provider acceptance is neither device receipt nor reading, and uncertain retry can duplicate. Existing recurring grace/confirmation semantics remain unchanged. Schema, evidence and deployment hold: [Task 047](../agent-tasks/047-one-shot-reminders.md).
