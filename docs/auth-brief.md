...
- **Cron endpoints:** Đã xoá toàn bộ external API cho cron (kể cả heartbeat và webhook). Thay vào đó, app chạy một in-process async timer (`011d`) không phơi bề mặt mạng ra ngoài nên không cần authen API.
