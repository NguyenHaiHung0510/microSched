...
## 10. Readability update — 2026-09-06, Task 043

Within the Owner-delegated UI/UX task, the logo is a keyboard/touch-accessible home link that returns to the default Task view through SPA navigation. Calendar can use the wider laptop surface; other views retain their existing reading width. Desktop month chips use two text lines with the existing full day detail path. A selectable, remembered month agenda shows a compact date picker and full event/task text, while preserving grid and the separate event list. Its month navigation loads the selected range; loading/error states are distinct from empty days and failed quick-add retains the draft. The grid heading follows the central visible week below measured sticky controls.

Private objects that have already passed the existing read gate use a lock label, a strong leading edge and the existing warm rose tokens. Pinned state retains its own pin indication. This is presentation only; it does not change unlock lifetime, authorization, cache boundaries or encryption. Task dates with no rows under the selected filter use compact adjacent cells; populated days retain full-width groups, loaded ranges and history navigation. Mobile main tabs remain one row. All original design decisions and unrun device acceptance boundaries still apply.

## 11. Reminder management — Owner approved 2026-09-07, Task 047

The global bell opens a desktop side panel or full mobile panel: Upcoming, Attention and collapsed History. Source-local “Nhắc tôi” in a saved task/event/tracker opens a one-shot editor with an enable toggle, custom datetime, before/after units, quick presets and resolved Vietnam time. Unsaved objects are saved first. Date-only sources ask for an explicit clock. Existing recurrence stays in the tracker editor and is visible from the center.

Device registration is an explicit user action available in the center/editor; saving a schedule does not request browser permission. Zero registered devices is visibly explained. Errors retain the draft and mutation success does not wait for background cache refetches. Source-derived private data and open editors clear through the existing lock/expiry lifecycle. Physical iPhone/Safari and real Web Push are separate acceptance, not implied by Chromium viewports.
