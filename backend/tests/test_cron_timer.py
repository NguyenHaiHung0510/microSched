...
async def test_item_exception_does_not_kill_loop(monkeypatch):
    """011d §6.2: one failing item must not kill the timer loop."""
    ...
    timer = CronTimer(FakeFactory(BoomDB(results=[[], [], []])))
    item = TimerItem(
    ...
    assert timer.status == "degraded"  # initial snapshot failed, not the loop
    await timer.stop()
...
@pytest.mark.anyio
async def test_unexpected_loop_failure_logs_and_recovers(monkeypatch):
    """Bacon-F2 (011d §5.3): top-level failure → cron_timer_loop_failed + degraded
    + bounded backoff; the loop survives instead of dying silently."""
    ...
    class _FailingTZ(tzinfo):
        ...
        def __init__(self):
            self.remaining_failures = 5

        def utcoffset(self, dt):
            if self.remaining_failures > 0:
                self.remaining_failures -= 1
                raise RuntimeError("synthetic top-level clock failure")
    ...
    # datetime.now(VN_TZ) inside the run() body is a top-level call; no
    # item-level handler sees it, so it exercises the Bacon-F2 catch-all.
    # The first failure lands in the startup snapshot (degraded, no counter),
    # the second in the loop body: _loop_failures must rise and the loop must
    # keep running after the bounded backoff. A pre-seeded heap item cannot
    # exercise this: load_snapshot replaces the heap wholesale on startup.
    monkeypatch.setattr(cron, "VN_TZ", _FailingTZ())

    task = asyncio.create_task(timer.run())
    await asyncio.sleep(0.15)
    assert task.done() is False, "the loop must survive an unexpected top-level failure"
    assert timer._loop_failures >= 1
