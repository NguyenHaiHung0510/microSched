...
async def test_scheduler_auto_reconnect_on_ownership_loss(monkeypatch):
    ...
    assert timer._lock_connection is not None
    assert timer._lock_connection.listener is not None
    await asyncio.sleep(0.01)

    # Simulate connection drop
    acquired_event.clear()
    timer._lock_connection.listener(timer._lock_connection)

    # Verify that timer reconnects and acquires lock again
    await asyncio.wait_for(acquired_event.wait(), timeout=1)
