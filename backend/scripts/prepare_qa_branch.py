    ...
    try:
        async with conn.transaction():
            ...
            tasks = await conn.fetch(
                'SELECT id, title, body_md, is_private FROM microsched.task'
            )
            ...
            for t in tasks:
                ...
                if is_priv:
                    ...
                    new_body = (
                        _encrypt_value(qa_key, scramble_text(body_plain, salt))
                        if t_body
                        else None
                    )
                else:
            ...
            for ti in task_items:
                ...
                if is_priv:
                    ...
                    new_content = _encrypt_value(
                        qa_key, scramble_text(content_plain, salt)
                    )
                else:
            ...
            notes = await conn.fetch(
                'SELECT id, title, body_md, is_private FROM microsched.note'
            )
            ...
            trackers = await conn.fetch(
                'SELECT id, name, reminder_text FROM microsched.tracker'
            )
            ...
            for tr in trackers:
                ...
                rem_text = (
                    scramble_text(tr['reminder_text'], salt)
                    if tr['reminder_text']
                    else None
                )
                await conn.execute(
            ...
            sources = await conn.fetch(
                'SELECT id, display_name FROM microsched.calendar_source'
            )
            ...
            for ce in cal_events:
                ...
                c_desc = (
                    scramble_text(ce['description_md'], salt)
                    if ce['description_md']
                    else None
                )
                c_loc = (
                    scramble_text(ce['location'], salt) if ce['location'] else None
                )
                ev_sql = (
            ...
            for da in day_annots:
                ...
                d_note = (
                    scramble_text(da['note_md'], salt) if da['note_md'] else None
                )
                await conn.execute(
...
def main() -> None:
    ...
    parser.add_argument(
        '--pin', default='123456', help='Test PIN to set for private unlock'
    )
    parser.add_argument(
    ...
    for k, v in res.items():
        print(f'  - {k}: {v}')
    print(
        'Pre-seeded QA Session cookie: ms_session=qa_token '
        '(user: owner@test.local, PIN: 123456)'
    )
