...
- **Pre-commit hooks**: This repo uses `pre-commit` to prevent committing secrets (`gitleaks`) and to enforce basic file hygiene. The git hooks are not installed automatically upon cloning. After cloning the repo, you **must** run `pre-commit install` to activate them.
