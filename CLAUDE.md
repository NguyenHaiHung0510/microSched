...
**Adaptive intake/quality contract:** cụm issue thô chưa phải task để dispatch. T1 phải triage, tư vấn và chốt với chủ task contract trước first write: slice/scope, dependency, topology/model, quality axes, review posture, report depth, loop budget và stop/re-scope trigger. Không có cap số vòng chung cho mọi task; review bind immutable target, đi hết declared axes và trả consolidated ledger. Product correctness, evidence/report và runtime/device là các gate riêng. Chi tiết nằm trong task spec; decision framework hiện hành ở `docs/devops-brief.md` §7.

**`coordination_record` fail-closed.** Delegation/experiment cấp authority architecture/product/irreversible phải có một UTF-8 JSON tracked tại `agent-tasks/coordination-records/<record_id>.json`; authority của record chỉ có khi independent validator bind đúng `authority_binding` với approval/policy bất biến được trust độc lập, theo schema ở `docs/devops-brief.md` §7. Record tự nó không phải authority, không được tự mở rộng scope/action/expiry hay suy authority từ approval cũ; thiếu/mismatch/không resolve được là deny. Experiment không tự nâng topology/authority thành policy dù kết quả tốt.
